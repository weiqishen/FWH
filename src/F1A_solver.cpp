...
void F1A_solver::calc_pressure_term(double endt, double begint)
{
	...
	ndarray<double> Lr;		 // Lr = Li (dot) r -> Li = d(p)/dt and r = radiation vector from source faces to microphone location
	ndarray<double> rhoCalc; // Interpolated density at source
	ndarray<double> Machr;   // M (dot) r of flow -> M = Mach number and r = radiation vector from source faces to mic location
	ndarray<double> un;		 // u (dot) n of flow -> Normal velocity
	ndarray<double> ur;		 // u (dot) r of flow -> velocity in radiation direction
	...
	for (size_t j = 0; j < faces.n_eles; j++)
	{
		...
		for (size_t timeloop = 0; timeloop < t.get_len(); timeloop++) //for each observer time step
		{
			...
			while (counter < faces.tau.get_len() - 1)
			{
				if (tauCalc > faces.tau(counter) && tauCalc <= faces.tau(counter + 1))
				{
					...
					//p\hat{n}
					L(0) = pressure * faces.normal(j);
					L(1) = pressure * faces.normal(j);
					L(2) = pressure * faces.normal(j);
		...
		for (size_t timeloop = 0; timeloop < t.get_len(); timeloop++) //for each observer time step
		{
			...
			{
				dlr_dt = secondOrderCentralDiff(Lr(timeloop - 1), Lr(timeloop + 1), faces.dt);
				dMachr_dt = secondOrderCentralDiff(Machr(timeloop - 1), Machr(timeloop + 1), faces.dt);
				dun_dt = secondOrderCentralDiff(un(timeloop - 1), un(timeloop + 1), faces.dt);
				drho_dt = secondOrderCentralDiff(rhoCalc(timeloop - 1), rhoCalc(timeloop + 1), faces.dt);
			}
			else if (timeloop == 0)
			{
				dlr_dt = secondOrderBackwardDiff(-Lr(timeloop), -Lr(timeloop + 1), -Lr(timeloop + 2), faces.dt);
				dMachr_dt = secondOrderBackwardDiff(-Machr(timeloop), -Machr(timeloop + 1), -Machr(timeloop + 2), faces.dt);
				dun_dt = secondOrderBackwardDiff(-un(timeloop), -un(timeloop + 1), -un(timeloop + 2), faces.dt);
				drho_dt = secondOrderBackwardDiff(-rhoCalc(timeloop), -rhoCalc(timeloop + 1), -rhoCalc(timeloop + 2), faces.dt);
			}
			else
			{
				dlr_dt = secondOrderBackwardDiff(Lr(timeloop), Lr(timeloop - 1), Lr(timeloop - 2), faces.dt);
